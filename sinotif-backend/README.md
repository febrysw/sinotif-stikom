...
GraphQL Playground for the NestJS Server is available here: http://localhost:3333/graphql
...
[RESTful API](http://localhost:3333/api) documentation available with Swagger.
...
```bash
docker run -d -t -p 3333:3333 --env-file .env nest-prisma-server
```

Now open up [localhost:3333](http://localhost:3333) to verify that your nest server is running.
